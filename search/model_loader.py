...
def get_embedding_model(device: Optional[str] = None, model_name: str = 'all-MiniLM-L6-v2'):
    """
    ...
    # Check if we have a cached model for the same device
    with _model_lock:
        if _model_cache is not None and _model_device == device:
            logger.debug(f"Reusing cached model on device: {device}")
            return _model_cache
        
        # Load model (first time or device changed)
